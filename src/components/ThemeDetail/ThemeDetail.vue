  ...
  <div class="themeDetail">
    <div class="model" :class="model">
      ...
      <div class="swipe">
        <mt-swipe :auto="4000">
          <mt-swipe-item> 
            <img :src="attachImageUrl(this.$store.state.currentTheme.image)" >
            <span class="title" >{{this.$store.state.currentTheme.description}}</span>
          </mt-swipe-item>
        </mt-swipe>
      </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods:{
      ...
      //跳转主编列表路由
      showEditors(id) {
          router.push({ name:'editorsList',params:{id}})
      },
      //前往主题新闻详情页
      goNew(id) {
        this.$store.state.id = id;
        router.push({ name:'newDetail', params:{ id }});
