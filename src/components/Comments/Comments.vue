  ...
  export default {
    ...
    methods:{
      date(time){
        ...
        let commonDate = new Date(time);
        
        let month = commonDate.getMonth()+1;
        let date = commonDate.getDate();
        let hour = commonDate.getHours();
        let minute = commonDate.getMinutes();
        month = month <10 ?'0'+month : month;
        date = date <10 ?'0'+date :date;
        hour = hour <10 ?'0'+hour :hour;
        minute = minute <10 ?'0'+minute :minute;
        let strDate =  month+ '-' +date +' ' + hour +':'+minute;
        return strDate
      },
      //获取长评论数据
      fetchLongData() {
